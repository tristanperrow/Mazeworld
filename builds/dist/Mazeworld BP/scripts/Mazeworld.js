 ...
 *
*/ let TPS = 20;
/** To work with main.js (idk why having one export makes this all work) */ export let dim = server.world.getDimension("overworld");
let settingsPresets = settingPresetsData;
/** The maze generation options from maze_generation.ts */ let mazeGenerationOptions = {
    wallType: "glass",
    towerDifficulty: "easy",
    lootQuality: "strong",
    waterZone: "on",
    mazeSize: server.world.getDynamicProperty("lastMazeSize") || 123,
    playerCount: 12
};
/** The item effect options from items.ts */ let itemOptions = {
    time_between_set_effects: 1.0,
    duration_hit_effects: 0.5,
    duration_hit_lingering_effects: 4.0,
    gun_damage: 7
};
/** List of all current players in a game  */ export let currentPlayers = [];
/** Amount of times the water zone has spread this game */ export let stormCount = server.world.getDynamicProperty("stormCount") || 0;
/** The exponential factor at which the storm slows down by maze size. */ export let stormFactor = 1.4;
/*
 *
 * FUNCTIONS
 * 
 ...
 */ function stormNextTime(count) {
    if (count == 0) return stormStartTime();
 ...
 */ function clearInventory(player) {
    let plrInv = player.getComponent(server.EntityComponentTypes.Inventory);
    let plrEqp = player.getComponent(server.EntityComponentTypes.Equippable);
    // clear inventory
 ...
 */ async function startGame() {
    ...
    for(let x = 0; x < playerList.length; x++){
        ...
        player.addEffect("saturation", 10, {
            amplifier: 10
        }); // give saturation
        player.addEffect("regeneration", 10, {
            amplifier: 10
        }); // give regen
 ...
 */ function tryGameOver() {
    ...
    for (const player of currentPlayers){
        if (server.world.getPlayers({
            name: player.name
        })[0] && server.world.scoreboard.getObjective("currentgame").getScore(player) == 1) numPlayersRemaining++;
    }
 ...
 * Events 
 *  
 * */ /* when player joins, update adjMinMazeSize and playerCount */ server.world.afterEvents.playerJoin.subscribe((event)=>{
    updateMazeSize("0");
    updatePlayerCount("1");
});
/* when player leaves, remove from current game if possible */ server.world.beforeEvents.playerLeave.subscribe((event)=>{
    if (!isGameActive()) return;
    if (!server.world.scoreboard.getObjective("currentgame").hasParticipant(event.player)) return;
    tryGameOver();
});
/* /scriptevent commands */ server.system.afterEvents.scriptEventReceive.subscribe((event)=>{

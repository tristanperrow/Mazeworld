...
/** The integer given to the set effect interval to be used for resetting the set effect interval. */ export let runIntervalInteger = null // NECESSARY FOR SETTINGS
;
/** The ticks per second of a minecraft server (should always be 20). */ let TPS = 20;
/** The time (in seconds) between when set effects are re-applied / taken away from the player. */ let time_between_set_effects = 1.0;
/** The duration (in seconds)  of a hit effect to disallow for swapping between weapons. */ let duration_hit_effects = 0.5;
/** The duration (in seconds) of a lingering hit effect, for effects like poison and fire. */ let duration_hit_lingering_effects = 4.0;
/** The damage of the blicky, in half-hearts(?). */ let gun_damage = 7;
/*
 *
 ...
 */ function startInterval() {
    return server.system.runInterval(()=>{
        const players = [
            ...server.world.getPlayers()
        ];
        for (const player of players){
            let applySetBonus = hasArmorSet(player);
            if (applySetBonus != null) {
                for (const effect of applySetBonus.armorSet.effects){
                    player.addEffect(effect.type, TPS * time_between_set_effects * 5, {
                        amplifier: effect.amplifier
                    });
                }
            }
            let plrEquipComp = player.getComponent(server.EntityComponentTypes.Equippable);
 ...
 */ function checkItemAmount(player, itemId) {
    const component = player.getComponent(server.EntityComponentTypes.Inventory);
 ...
 */ function getEquippedArmor(player) {
    const equipment = player.getComponent(server.EntityComponentTypes.Equippable);
    return {
 ...
 */ function hasArmorSet(player) {
    let itemIds = getEquippedArmor(player);
    for (const [setName, armorSet] of Object.entries(setBonuses)){
        if (armorSet.helmet === itemIds.helmet && armorSet.chestplate === itemIds.chestplate && armorSet.leggings === itemIds.leggings && armorSet.boots === itemIds.boots) {
            return {
                name: setName,
                armorSet: armorSet
            };
 ...
 */ function hasSpecificArmorSet(player, setName) {
    let itemIds = getEquippedArmor(player);
    //console.warn(setBonuses[setName]);
    if (setBonuses[setName] == null) return false;
    if (setBonuses[setName].helmet === itemIds.helmet && setBonuses[setName].chestplate === itemIds.chestplate && setBonuses[setName].leggings === itemIds.leggings && setBonuses[setName].boots === itemIds.boots) {
        return true;
    }
    return false;
}
 ...
 */ function applyHitEffect(player, target, effect) {
    if (!effect) return;
 ...
 */ function applyPassiveEffect(player, effect) {
    if (!effect) return;
    switch(effect.type){
        case "effect":
            if (!effect.effectType) {
 ...
 */ function applyHitEffects(player, target, itemId) {
 ...
 */ function applyPassiveEffects(player, itemId) {
    // check if weapon has a weapon effect object
    const effectObject = weaponEffects[itemId];
    if (!effectObject) return;
    for (const effect of effectObject.effects){
...
/* Hitscan Glock event */ server.world.afterEvents.itemUse.subscribe(shootGlock);
/* For special weapon effects */ server.world.afterEvents.entityHitEntity.subscribe((event)=>{
    if (event.damagingEntity.typeId != "minecraft:player") return;
    let entEquipComp = event.damagingEntity.getComponent(server.EntityComponentTypes.Equippable);
    let handItem = entEquipComp.getEquipment(server.EquipmentSlot.Mainhand);
    if (!handItem) return;
    applyHitEffects(event.damagingEntity, event.hitEntity, handItem.typeId);
});
/* FINAL STUFF */ runIntervalInteger = startInterval();
